...
<div class="container  mt-4">
    <div class="row">
    <h1>Atualização de Paciente</h1>
    <hr>
    </div>
    <form action="{{ route('pacientes.atualizar') }}" method="post" class="row g-3">
      @csrf
      <input type="hidden" name="cod_paciente" value="<?php echo $paciente->cod_paciente ?>">
      <input type="hidden" name="data_cadastro" value="<?php echo  $paciente->data_cadastro ?>">
    <div class="col-md-4">
        <label for="" class="form-label">Nome do  Paciente:</label>
        <input type="text"  name="nome_paciente" class="form-control" value="{{ $paciente->nome_paciente }}">
    </div>
    <div class="col-md-4">
    <label for="" class="form-label">Telefone:</label>
    <input type="number" name="tel_paciente" class="form-control" value="{{ $paciente->tel_paciente }}" >
  ...
  <div class="col-md-4">
    ...
    <div class="input-group ">
      <input type="text" name="carterinha" class="form-control" id="" aria-describedby="" value="{{ $paciente->carterinha }}">
  ...
  <div class="col-md-3">
    ...
    <select class="form-select" name="cidade_paciente" id="" aria-describedby="validationServer04Feedback">
      <option selected value="{{ $paciente->cidade_paciente }}">{{ $paciente->cidade->estado }}</option>
  ...
  <div class="col-md-3">
    ...
    <select class="form-select " name="sexo_paciente" id="" aria-describedby="validationServer04Feedback" >
      <option selected value="{{ $paciente->sexo_paciente }}">{{ $paciente->sexo->sexo }}</option>
  ...
  <div class="col-md-3">
    ...
    <select class="form-select" name="orientacao_paciente" id="validationServer04" aria-describedby="validationServer04Feedback">
      <option selected value="{{ $paciente->orientacao_paciente }}">{{ $paciente->orientacao->orientacao }}</option>
  ...
  <div class="col-md-3">
    ...
    <select class="form-select is-invalid" name="estado_civil_paciente" id="validationServer04" aria-describedby="validationServer04Feedback" >
      <option selected value="{{ $paciente->estado_civil_paciente }}">{{ $paciente->estadoCivil->estado_civil }}</option>
  ...
  <div class="col-md-3">
    ...
    <select class="form-select is-invalid" name="convenio_paciente" id="validationServer04" aria-describedby="validationServer04Feedback">
      <option selected value="{{ $paciente->convenio_paciente }}">{{ $paciente->convenio->convenio }}</option>
  ...
  <div class="col-md-3">
    ...
    <select class="form-select is-invalid"  name="abordagem_paciente"id="validationServer04" aria-describedby="validationServer04Feedback">
      <option selected value="{{ $paciente->abordagem_paciente }}">{{ $paciente->abordagem->Abordagem }}</option>
